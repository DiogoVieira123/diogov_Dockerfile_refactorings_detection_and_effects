...
| --- | --- | --- | --- | --- |
| **Exp. 1** | Validate the three-axis measurement principle by applying four supported refactorings in isolation and confirming that the four deltas (Delta Size, Delta CVEs, Delta Warnings, Delta Instr) capture distinct impact profiles, where some refactorings affect more than one dimension and others affect only one. | Apply each of four supported refactorings, Inline RUN Instructions (R01), Update Base Image TAG (R02), Update Base Image (R10), and Replace ADD with COPY (R08) , in isolation to a minimal Dockerfile pair, build both images, and measure the four deltas for each pair. | The four refactorings produced distinct impact profiles, measured with Hadolint unfiltered. Inline RUN Instructions (R01) reduced image size by 194,991 bytes and the logical instruction count by three, while the raw warning total fell from 10 to 3: DL3059 (3 → 0), the smell the refactoring targets, together with DL3008 (3 → 1) and DL3015 (3 → 1), which fall because four apt-get invocations became one. Update Base Image TAG (R02) eliminated the DL3007 latest-tag smell, taking the raw total from 1 to 0. Update Base Image (R10, ubuntu:22.04 to alpine:3.19) reduced image size by approximately 26.3 MB and removed 15 CVEs, improving both performance and security, with no warning in either state. Replace ADD with COPY (R08) eliminated a single DL3020 warning with a minor size increase of 3 bytes and no security impact. No refactoring degraded a dimension it was not intended to affect, so the trade-off flag was not triggered by this set. | The four-metric measurement principle. |
...
| **Exp. 4** | Validate precise image size retrieval through the Docker daemon API rather than CLI text parsing. | Build a Dockerfile image and read image.attrs\['Size'\] via the Docker SDK for Python. | Returned 29,748,045 bytes (28.37 MB) as a raw integer, eliminating CLI rounding imprecision. | Performance Analyzer |
...
Table 4.1 - Summary of the five rapid experiments, their objectives, methods, results, and the system components whose design they ground.
...
The third challenge is consolidation-versus-deletion ambiguity. When the count of RUN instructions decreases between two commits, the change may represent a pure consolidation, a pure deletion, or a mixed commit combining instruction inlining with concurrent code cleanup. These operations have distinct structural implications. To accommodate real-world developer practices where refactorings and minor cleanups frequently overlap, the Detection Engine employs a fusion-proof criterion rather than a strict preservation check. It searches for positive evidence of instruction merging: if the primary command blocks are effectively combined into a surviving instruction, the change is classified as a consolidation refactoring and triggers the R01 rule even if minor adjacent commands or cleanups were simultaneously deleted. Conversely, if the reduction in instruction count stems from a pure deletion without any structural fusion, the change is classified as a deletion and is not reported as a refactoring.
...
RNF2 — Diagnostic Sensitivity: The two maintainability instruments shall each be sensitive enough to detect a change in the property they measure. At the smell level, Hadolint shall detect the complete elimination of a relevant rule violation; at the structural level, the parser shall detect the addition or removal of a single logical instruction. Sensitivity is a property of the instruments and not a claim of coverage: as stated in Section 4.1.3, neither indicator spans the full set of maintainability sub-characteristics defined by ISO/IEC 25010 \[25\], and the requirement asserts only that each responds to the changes it is designed to register. Acceptance criteria: PoC-2 test — Delta Warnings(DL3007) = -1 correctly detected, representing complete elimination of the mutable-tag smell between Dockerfile A (ubuntu:latest) and Dockerfile B (ubuntu:22.04); R06 test — Delta Instr = -3 correctly detected on a refactoring that registers an improvement on the maintainability axis.

RNF3 — Extensibility: The Detection Engine rule set shall be extensible to new refactoring types without modification of existing component code, following the Open-Closed Principle \[26\]. Each rule is self-contained and registered in a declarative rule list, so that adding a type requires an addition to that list and no change to the execution logic of any component. The prototype supports fourteen types; the architecture must accommodate additional types in future iterations. Acceptance criterion: a new detection rule can be added to the registry and exercised end to end without editing any existing rule or any downstream component.
...
RNF5 — Reproducibility: The system shall record, in every report, the analysis timestamp, the version of each external tool invoked (Docker Engine, Hadolint, Trivy, dockerfile-parse), and the Trivy vulnerability database version, enabling future re-runs to identify whether differences in CVE counts are attributable to Dockerfile changes or to database updates. Furthermore, reproducibility is ensured not only by these records but also through the capability to retain the source analysis artifacts generated by external tools during the extraction phase. Acceptance criterion: every generated report contains the analysis timestamp, the tool version block, the Trivy vulnerability database version, and the retained source analysis artifacts.

### Architecture Design

The prototype tool is structured as a sequential data pipeline in which each stage transforms a well-defined input into a well-defined output consumed by the next stage. This architecture was chosen over alternative approaches, such as an event-driven or service-oriented design, because the analysis is inherently sequential at the coarse level: version extraction must precede detection, and detection must precede the empirical evaluation of the two Dockerfile states. The pipeline architecture also ensures deterministic, reproducible execution consistent with RNF5.
...
Figure 4.1 - Sequential architecture of the prototype analysis pipeline, showing the execution order of the components. The data dependencies between components are detailed in Figure 4.2.

At the metric collection stage, between the Detection Engine and the Report Generator, the three quality dimensions are measured in parallel. The Data Extractor dispatches Hadolint (Maintainability) and Trivy (Security) concurrently while the Performance Analyzer retrieves the image size via the Docker SDK. Since these three operations are mutually independent, none requires the result of another, parallel execution is the natural architectural choice, reducing the total metric collection time from the sum of three sequential operations to approximately the duration of the slowest one. Each branch produces a typed metric object that is aggregated by the Report Generator. The fourth delta, ΔInstr, is produced by the Data Extractor as well, but it does not occupy a branch of its own: it requires no external tool invocation, since it is a direct count over the instruction list the component parses from the Dockerfile text itself, and its cost is negligible against the build and scan operations that dominate this stage. Neither branch consumes the output of the Detection Engine: both operate on the Dockerfile pair produced by the VCS Connector, so the deltas are obtained without any knowledge of which refactoring was detected. The detected identifier and the measured deltas meet only at the Report Generator, where they are paired in the final report.

To complement the execution sequence shown in Figure 4.1 with an explicit view of the components and the data exchanged between them, Figure 4.2 presents the component diagram of the prototype, showing each module, its provided and required interfaces, the dependencies between modules, and the external tools used.

Figure 4.2 - Component diagram of the prototype: modules, provided and required interfaces, dependencies, and external tools

Figure 4.2 presents the component diagram of the prototype, showing the flow of data between components rather than their temporal execution order. Each component provides a typed object, shown as the label on its outgoing dependency, and requires the objects on its incoming dependencies, following the data transformations defined in Table 4.10. The VCS Connector provides the two Dockerfile versions (Dockerfile_A and Dockerfile_B), which are required by the Detection Engine, the Performance Analyzer, and the Data Extractor. The Detection Engine provides a DetectionResult, carrying the catalogue identifier of the transformation and the instructions involved, which is required directly by the Report Generator. The Performance Analyzer provides the image sizes and the Data Extractor provides the metric sets covering warnings, vulnerabilities, and the logical instruction count. The Report Generator requires the DetectionResult, the sizes, and the metric sets, and produces the ImpactReport as its output artifact. The diagram makes the separation between detection and measurement explicit: the Performance Analyzer and the Data Extractor have no incoming dependency on the Detection Engine, so no expected impact is available to them while they measure. The dashed dependencies show the external tools each component uses: GitPython (VCS Connector), dockerfile-parse (Detection Engine and Data Extractor), the Docker SDK and the Docker daemon (Performance Analyzer), and Hadolint and Trivy (Data Extractor). The Report Generator relies only on internal rule-based logic and uses no external tool.

The interaction between the user and the prototype is intentionally simple. Figure 4.3 presents the use case diagram, capturing what the user can do with the tool.

Figure 4.3 - Use case diagram of the Dockerfile refactoring analysis tool.
...
Figure 4.4 shows the deployment view of the prototype, identifying where each part of the system executes at runtime.

Figure 4.4 - Deployment diagram of the prototype, showing the Python application, the Docker daemon, and the tool and image containers.

The prototype runs as a Python application on the host machine and delegates all container operations to the Docker daemon, with which it communicates through the Docker SDK over the Unix socket. The Hadolint and Trivy tools execute as containers managed by this daemon, alongside the images built from Dockerfile_A and Dockerfile_B. Hadolint analyzes the Dockerfile text, while Trivy scans the two built images. Running the tools as containers keeps their versions pinned and removes the need for host-level installation, which supports the reproducibility of the analysis.

To provide a complete overview of the entire architecture, Table 4.2 summarizes the responsibilities of every system component, the technologies used for their implementation, and the requirements they address:

| **Component** | **Responsibility** | **Validated Technology** | **Satisfies Requirements** |
| --- | --- | --- | --- |
| **VCS Connector** | Extracts Dockerfile content from the Git object model | GitPython — Exp. 3 | RF1, RNF4, RNF5 |
| **Detection Engine** | Parses logical instructions, identifies the refactoring type, and emits its formal identifier | dockerfile-parse — Exp. 5 | RF2, RF3, RNF3 |
| **Performance Analyzer** | Builds image and retrieves size in bytes via daemon API | Docker SDK — Exp. 4 | RF4, RNF1, RNF4 |
| **Data Extractor** | Invokes static and security tools, counts logical instructions, and normalises metrics | Hadolint + Trivy — Exp. 2; dockerfile-parse | RF5, RF6, RNF2, RNF4, RNF5 |
| **Report Generator** | Aggregates the detected identifier with the metric deltas and produces the ImpactReport | Rule-based computation | RF7, RNF5 |

Table 4.2 - Prototype components, their responsibilities, technologies, and satisfied requirements.
...
At the system level, end-to-end behavior is validated against the four controlled multi-dimension experiment scenarios (R06, R09, R11, and R12), all executed using an alpine:3.20 base image. The expected reference values are:

- **R06 (Inline Stage)** — (3,632,710 → 3,632,826), , , (8 → 5);
- **R09 (Extract RUN Instructions)** — (3,633,775 → 3,634,192), , , (5 → 6);
- **R11 (Move Stage)** — (3,632,724 → 3,632,725), , , (8 → 9);
- **R12 (Remove RUN (mv))** — (3,634,927 → 3,633,596), , , (8 → 6).
...
| **R11** | Move Stage | X   | X   | ↑   |
| **R12** | Remove RUN Instruction (mv) | X   | X   | ↑   |
| **R13** | Update RUN Instruction | X   | X   | ↑   |
| **R14** | Rename Image | X   | X   | ↑   |

Table 4.3 - Extended Dockerfile refactoring catalogue mapping fourteen types across three quality dimensions
...
**Performance X.** The controlled experiment on alpine:3.20 measured an increase of 417 bytes (3,633,775 → 3,634,192), the largest residual variation in the catalogue, determined by the external script file added to the image and the manifest cost of the additional COPY directive. Although the most pronounced of the metadata-scale results, it remains three orders of magnitude below the operational threshold and is therefore classified as neutral.
...
Each refactoring is evaluated independently across all three quality axes, Performance, Security, and Maintainability, forming a complete three-dimensional impact vector. This approach provides a transparent overview of positive effects, neutral behaviors, and trade-offs introduced by each transformation across all axes without relying on a primary-versus-secondary hierarchy.
...
Table 4.4 - Version control system access methods considered for the VCS Connector
...
Table 4.5 - Dockerfile parsing strategies considered for the Detection Engine
...
| **ID** | **Refactoring Type** | **Hadolint (smell fixed)** \[10\] | **DRMiner** \[9\] | **Docker-Parfum** \[22\] | **Implemented in Prototype** |
| --- | --- | --- | --- | --- | --- |
| **R01** | Inline RUN Instructions | DL3059 | Yes | No  | Yes |
| **R02** | Update Base Image TAG | DL3007 | Yes | No  | Yes |
| **R03** | Add ENV Variable | —   | Yes | No  | Yes |
| **R04** | Add ARG Instruction | —   | Yes | No  | Yes |
| **R05** | Extract Stage | —   | Yes | No  | Yes |
| **R06** | Inline Stage | —   | Yes | No  | Yes |
| **R07** | Sort Instructions | —   | Yes | No  | Yes |
| **R08** | Replace ADD with COPY | DL3020 | Yes | Yes | Yes |
| **R09** | Extract RUN Instructions | —   | Yes | No  | Yes |
| **R10** | Update Base Image | —   | Yes | No  | Yes |
| **R11** | Move Stage | —   | Yes | No  | Yes |
| **R12** | Remove RUN (mv command) | —   | No  | No  | Yes |
| **R13** | Update RUN Instruction | —   | No  | No  | Yes |
| **R14** | Rename Image | —   | Yes | No  | Yes |

Table 4.6 - Relationship between the fourteen catalogue refactorings and the reviewed tools (Hadolint reports smells; DRMiner detects refactorings), with the implementation decision for the prototype

## Performance Analyzer

### Technical Description

To retrieve image metadata within the containerized environment, the two official interfaces provided by the ecosystem were considered: the command-line interface (Docker CLI) and the software development kit (Docker SDK). In strict respect to the official Docker Engine API architecture \[34\], these two approaches represent the standard interfaces available to send instructions and interact with the Docker daemon. However, during the architectural investigation of these methods, a critical limitation in the Docker CLI regarding image size precision was identified. As documented by the developer community in Docker Issue #31298 \[35\], the CLI introduces decimal rounding and human-readable truncation (e.g., rounding byte counts to the nearest megabyte) by design. Since the non-functional requirement RNF1 demands byte-level precision to detect micro-variations and small size deltas, any reliance on the CLI was rejected.
...
| --- | --- | --- |
| **Docker CLI (--format "{{.Size}}")** | ❌ Rounded human-readable strings | ❌ Docker issue #31298 \[35\] |
| **Docker SDK (selected)** | ✅ Raw 64-bit integer bytes | ✅ Experiment 4 |

Table 4.7 - Image size retrieval methods considered for the Performance Analyzer
...
While a standard pipeline design might measure image size using the Docker CLI command docker images --format "{{.Size}}", technical documentation and open issues (such as Docker issue #31298) indicate that this approach returns a pre-parsed, human-readable string subject to rounding imprecision. To bypass this limitation from the start, the Performance Analyzer replaces any reliance on the CLI with a direct SDK call that returns the raw integer byte count, completely eliminating the human-readable truncation risk
...
Table 4.8 - Tool invocation strategies considered for the Data Extractor.
...
Instr is reported under a contextual reading rather than as a scalar regression indicator. As set out in Section 4.1.3, refactorings act on the instruction count in opposite directions by design: consolidation and removal reduce it, whereas extraction and addition increase it. Consequently, a positive Instr is not inherently interpreted as a quality degradation. The report presents Instr alongside the specific refactoring type that produced it, ensuring the metric is evaluated against the nature of the applied operation rather than against an arbitrary fixed target.
...
Table 4.9 - Output formats considered for the Report Generator.
...
The Report Generator must satisfy RF7: produce a structured output presenting before-and-after values along with the computed metric deltas. The report is serialised in JSON format for downstream processing and in a human-readable text summary for direct review by practitioners. Both formats include the analysis timestamp and the commit SHAs to satisfy RNF5 (reproducibility).
...
| --- | --- | --- |
| **1** | **VCS Connector** | Inputs: repository path, SHA_A, SHA_B. Navigates the Git object model via GitPython. Output: Dockerfile_A, Dockerfile_B as text. |
| **2** | **Detection Engine** | Inputs: Dockerfile_A, Dockerfile_B. Parses each into a logical instruction list via dockerfile-parse and applies the self-contained detection rules. Output: DetectionResult carrying the catalogue identifier (R01–R14) and the instructions involved. |
| **3a** | **Performance Analyzer** | Inputs: Dockerfile_A, Dockerfile_B. Builds both Docker images and retrieves image.attrs\["Size"\] for each. Output: MetricSet_A, MetricSet_B (size in bytes). |
| **3b** | **Data Extractor** | Inputs: Dockerfile_A, Dockerfile_B and the built images. Invokes Hadolint and Trivy in parallel and counts logical instructions via dockerfile-parse. Output: MetricSet_A, MetricSet_B (warnings, CVEs by severity, instruction count). |
| **4** | **Report Generator** | Inputs: DetectionResult, MetricSet_A, MetricSet_B. Computes ΔSize, ΔCVEs, ΔWarnings, ΔInstr. Output: ImpactReport. |

Table 4.10 - Data transformations at each pipeline stage.
...
External variables entirely decoupled from the Dockerfile's syntax—such as transient network latency during RUN instructions, untracked updates within remote package manager mirrors (e.g., unexpected software upgrades during an apt-get execution), or silent baseline adjustments in external images maintained by public registries—can introduce confounding factors that temporarily perturb the collected performance ($\\Delta$Size) and security ($\\Delta$CVEs) indicators.
