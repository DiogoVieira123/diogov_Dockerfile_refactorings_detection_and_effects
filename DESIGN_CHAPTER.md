...
At the metric collection stage, between the Detection Engine and the Report Generator, the three quality dimensions are measured in parallel. The Data Extractor dispatches Hadolint (Maintainability) and Trivy (Security) concurrently while the Performance Analyzer retrieves the image size via the Docker SDK (utilizing the internal Image Builder mechanism to construct the container images). Since these three operations are mutually independent, none requires the result of another, parallel execution is the natural architectural choice, reducing the total metric collection time from the sum of three sequential operations to approximately the duration of the slowest one. Each branch produces a typed metric object that is aggregated by the Report Generator. The fourth delta, , is produced by the Data Extractor as well, but it does not occupy a branch of its own: it requires no external tool invocation, since it is a direct count over the instruction list the component parses from the Dockerfile text itself, and its cost is negligible against the build and scan operations that dominate this stage. Neither branch consumes the output of the Detection Engine: both operate on the Dockerfile pair produced by the VCS Connector, so the deltas are obtained without any knowledge of which refactoring was detected. The detected identifier and the measured deltas meet only at the Report Generator, where they are paired in the final report. To complement the execution sequence shown in Figure 4.1 with an explicit view of the components and the data exchanged between them, Figure 4.2 presents the component diagram of the prototype, showing each module, its provided and required interfaces, the dependencies between modules, and the external tools used.
...
| --- | --- | --- |
| **1** | VCS Connector | Input: Repository/SHA. Output: Dockerfile text. |
| **2** | Detection Engine | Input: Dockerfiles. Output: DetectionResult. |
| **3** | Performance Analyzer, Data Extractor | Input: Dockerfiles. Output: Metrics (MetricSet) / Deltas. |
| **4** | Report Generator | Input: DetectionResult + MetricSet. Output: ImpactReport (JSON). |
