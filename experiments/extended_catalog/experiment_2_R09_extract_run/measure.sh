#!/usr/bin/env bash
#
# measure.sh — R09 - Extract RUN Instructions
...
# The maintainability benefit (moving complex shell logic out of the Dockerfile) is argued from the nature of the operation; any Hadolint change is reported with its exact code for inspection.
#
# Usage:  bash measure.sh        (or: sudo bash measure.sh, if docker needs root)
# Output: printed to stdout AND saved to output.txt

set -u
BEFORE_TAG="r09-before:test"
AFTER_TAG="r09-after:test"
...
log "=================================================="
log " R09 - Extract RUN Instructions"
log " Experiment 2 - Extended Catalog"
